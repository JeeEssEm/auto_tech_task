...
class ContentNode(BaseModel):
    """Пользовательский подпункт ТЗ (до 3 уровней вложенности)."""
    id: str = Field(description="Уникальный идентификатор узла")
    title: str = Field(default="", description="Заголовок подпункта")
    content: str = Field(default="", description="Содержимое в формате markdown")
    children: list[ContentNode] = Field(
        default_factory=list, description="Дочерние подпункты",
    )
